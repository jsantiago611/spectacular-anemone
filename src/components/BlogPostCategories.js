            	...
            	_.map(categories, (category, category_idx) => {
            	    ...
            	    return (
                		category_data.link ? (<React.Fragment key={category_idx + '.1'}>
                			<Link style={{fontFamily:'tobias', fontWeight: "100", letterSpacing: -0.4}} key={category_idx} to={withPrefix(category_data.link)}>{category_data.title}</Link>{(!(category_idx === category_len - 1)) && (', ')}
